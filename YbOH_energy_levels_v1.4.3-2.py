...
gL = -1
gS = -2.0023
gJ = -0.8#-0.002 # Unsure exactly what order of magnitude gJ should come out to be, play with this parameter which also scales the hyperfine constants proportionally
gI = 5.585 * 1/1836.152672   # for H nucleus; using same muB for both terms, we divide this term by the difference in magnitude between muB and muN
...
B_scale = 1.3#2.6 Typical field we would produce is on the order of 2.5T
B_max = B_scale*fullfield(0,0,10e-3,stagescale)   

#z_position = 10.0e-3 #in units of mm
#z_position = 0.5e-3 #in units of mm
z_position = 0.1e-3 #in units of mm

B_val = B_scale*fullfield(0,0,z_position,stagescale)
Bfield = np.linspace(B_val,B_val,1)

z_position2 = 10e-3 #in units of mm
...
for i in range(len(excited_energies)):
...
plt.legend(handles=[red_line,blue_line])
plt.arrow(z_position/mm, ground_energies[1], 0, excited_energies[-3]-ground_energies[1], head_width=0.30, head_length=1.55, width = 0.06, length_includes_head = True)
#plt.arrow(z_position2/mm, ground_energies2[1], 0, excited_energies2[-3]-ground_energies2[1], head_width=0.30, head_length=1.55, width = 0.06, length_includes_head = True)

t = ("???")
plt.text(11.2, 11, t, fontsize=26, style='oblique', ha='center',rotation=15,va='top', wrap=True)

t1 = (r"$\vert \alpha \rangle$")
plt.text(10.2, -19, t1, fontsize=14, wrap=True)
...
#%%
print(transition_amplitude(excited_eigenvectors.H,ground_eigenvectors.H,q=0), '\n')
#print(transition_amplitude(excited_eigenvectors2.H,ground_eigenvectors2.H,q=0)[7])
