...
if __name__ == "__main__":
    num_particles = mp.cpu_count() * 85 #change the constant to adjust particle count.
    ics = []
...
for n in range(len(xt)):
    ax.scatter(xt[n][pos_initial]/(3*mm),vxt[n][pos_initial]/(sigma_v(3)), s = 10, color = 'red',alpha = 0.4)
    ax.scatter(xt[n][pos_final]/(3*mm),vxt[n][pos_final]/(sigma_v(3)), s = 10, color = 'blue',alpha = 0.4)
...
for n in range(len(xt)):
    ax.plot(zt[n]*1e3, kinetic_energy(vzt[n], vzt[n][0]) / kb)
ax.set_xlabel("z [mm]")
ax.set_ylabel("$KE$ [K]")
...
ax.grid()
for n in range(len(xt)):
    ax.scatter(zt[n][pos_initial] * 1e3,vzt[n][pos_initial] / (sigma_v(3)), s = 10, color = 'red', alpha = 0.4)
    ax.scatter(zt[n][pos_final] * 1e3,vzt[n][pos_final] / (sigma_v(3)), s = 10, color = 'blue', alpha = 0.4)
